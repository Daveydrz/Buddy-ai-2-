...
def apply_aec(mic_audio_bytes, ref_audio_bytes):
    if not USE_AEC:
        # Bypass AEC, just return the raw microphone data
...
class DummyAP:
    def process_reverse_stream(self, ref_bytes):
        pass
    def process_stream(self, mic_bytes):
        return mic_bytes

ap = DummyAP()  # Replace with: from webrtc_audio_processing import AudioProcessingModule
...
def set_ref_audio(audio_chunk):
    """
    Update the reference buffer with the latest chunk of audio played by Buddy (for AEC).
    This should be called for each small chunk sent to the speakers—typically 10ms at 16kHz (160 samples).
    """
    global ref_audio_buffer
    arr = np.frombuffer(audio_chunk, dtype=np.int16)
    # Downsample to 16kHz mono if needed
    arr = downsample(arr, MIC_SAMPLE_RATE, WEBRTC_SAMPLE_RATE)
    # Store only the most recent WEBRTC_FRAME_SIZE samples as the reference buffer
    with ref_audio_lock:
...
def cancel_mic_audio(mic_chunk):
    """
    Process a chunk of microphone audio using AEC, using the latest reference buffer.
    Always call this for each chunk of mic input, after updating the reference buffer with Buddy's playback.
    """
    mic = np.frombuffer(mic_chunk, dtype=np.int16)
    mic = downsample(mic, MIC_SAMPLE_RATE, WEBRTC_SAMPLE_RATE)
    # Feed the reference (what Buddy just played) to the AEC module
    with ref_audio_lock:
        ap.process_reverse_stream(ref_audio_buffer.tobytes())
    # Now process the mic audio
    result = ap.process_stream(mic.tobytes())
    return result

def downsample(audio, orig_sr, target_sr):
    """
    Downsample audio from orig_sr to target_sr, keeping only the first channel if stereo.
    """
    if audio.ndim > 1:
        audio = audio[:, 0]
...
def audio_playback_worker():
    ...
    while True:
        try:
            ...
            # Expect tuple: (samples, sample_rate)
            audio_np, sample_rate = item

            print("[DEBUG] Got from playback_queue:", type(audio_np), audio_np.shape, audio_np.dtype,
                  np.min(audio_np), np.max(audio_np), sample_rate)

            # Convert to stereo if mono (Windows soundcards may require this)
            if len(audio_np.shape) == 1:
                audio_np = np.stack([audio_np, audio_np], axis=1)
                print("[Buddy][Playback] Converted mono to stereo.")

            # Ensure type is int16
            if audio_np.dtype != np.int16:
                audio_np = audio_np.astype(np.int16)
                print("[Buddy][Playback] Converted dtype to int16.")

            print("[Buddy][Playback] Calling sd.play()")
            sd.play(audio_np, samplerate=sample_rate, device=SPEAKER_DEVICE_INDEX)
            sd.wait()
            print("[Buddy][Playback] sd.play() complete")
...
def play_chime():
    try:
        ...
        playback_queue.put((samples, audio.frame_rate))
    except Exception as e:
        if DEBUG:
    ...
    with sd.InputStream(device=MIC_DEVICE_INDEX, samplerate=MIC_SAMPLE_RATE, channels=1, blocksize=blocksize, dtype='int16') as stream:
        ...
        while True:
            ...
            for i in range(0, len(mic_16k), 160):
                ...
                if USE_AEC:
                    # --- SAFE: do NOT update ref_audio_buffer with Buddy's TTS; it is zeros or real loopback only
                    with ref_audio_lock:
                        ap.process_reverse_stream(ref_audio_buffer.tobytes())
                    processed = ap.process_stream(chunk.tobytes())
                    chunk_out = np.frombuffer(processed, dtype=np.int16)
                else:
                    chunk_out = chunk  # Bypass AEC
...
def generate_and_play_kokoro(text, lang=None):
    ...
    try:
        ...
        # Save debug wav at native sample rate
        debug_path = os.path.abspath("buddy_tts_debug.wav")
        sf.write(debug_path, samples, sample_rate, subtype='PCM_16')
        ...
        with playback_lock:
            ...
            if sample_rate != target_rate:
                print(f"[Buddy][TTS] Resampling TTS from {sample_rate} Hz to {target_rate} Hz for playback.")
                samples_resampled = resample_poly(samples, target_rate, sample_rate).astype(np.int16)
            else:
                samples_resampled = samples

            print(f"[DEBUG] Queuing audio to playback_queue, duration: {len(samples_resampled)/target_rate:.2f}s")
            playback_queue.put((samples_resampled, target_rate))
