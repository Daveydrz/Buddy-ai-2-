...
def identify_or_register_user(audio_chunk=None):
    """ENHANCED: Multi-user voice identification - Updated 2025-06-29 12:10:28"""
    global known_users
    
    print(f"[User] 🔍 Multi-user voice identification...")
    print(f"[User] Database: {len(known_users)} users: {list(known_users.keys())}")
    
    # If no database, register first user
    if len(known_users) == 0:
        print("[User] 📝 Empty database - registering first user")
        return register_new_user(audio_chunk)
    
    # Try voice recognition with LOWER threshold for better detection
    if audio_chunk is not None and len(audio_chunk) > 8000:
        try:
            print(f"[User] 🎤 Analyzing {len(audio_chunk)} audio samples...")
            ...
            if current_embedding is not None and len(current_embedding) == 256:
                print(f"[User] ✅ Generated voice embedding: {len(current_embedding)} dims")
                ...
                for name, stored_embedding in known_users.items():
                    try:
                        if isinstance(stored_embedding, list) and len(stored_embedding) == 256:
                            ...
                            print(f"[User] {name}: {similarity:.3f}")
                            
                            if similarity > best_score:
                                best_match = name
                                best_score = similarity
                    except Exception as comp_err:
                        print(f"[User] ❌ Error comparing {name}: {comp_err}")
                
                # ✅ LOWER threshold for better recognition in multi-user
                if best_match and best_score > 0.30:  # Even lower threshold
                    print(f"[User] ✅ VOICE RECOGNIZED: {best_match} (confidence: {best_score:.3f})")
                    
                    # Improve embedding
                    try:
                        old_embedding = known_users[best_match]
                        improved_embedding = [(old * 0.8 + new * 0.2) for old, new in zip(old_embedding, current_embedding)]
                        known_users[best_match] = improved_embedding
                        save_voice_database()
                        print(f"[User] 🔄 Enhanced voice profile for {best_match}")
                    except Exception as improve_err:
                        print(f"[User] ⚠️ Could not enhance profile: {improve_err}")
                    
                    set_last_user(best_match)
                    return best_match
                else:
                    print(f"[User] ❌ Voice not recognized (best: {best_match or 'None'}, score: {best_score:.3f})")
            else:
                print(f"[User] ❌ Invalid voice embedding")
                
        except Exception as voice_err:
            print(f"[User] ❌ Voice recognition error: {voice_err}")
    else:
        print(f"[User] ⚠️ No audio for voice recognition")
    
    # Not recognized - register new user
    print("[User] 👤 Unknown voice - registering new user")
    return register_new_user(audio_chunk)

def register_new_user(audio_chunk=None):
    """COMPLETELY FIXED: Register user with PERSISTENT voice saving"""
    global known_users
    ...
    # Wait for TTS to complete
    audio_queue.join()
    time.sleep(1.2)  # Longer wait for clear response
        ...
        try:
            ...
            if name_response and name_response.strip() != "...":
                ...
                if name_words:
                    ...
                    if name.lower() in ["david", "dave", "davey", "dav", "daveydrz", "david."]:
                        name = "Daveydrz"
                    elif name.lower() in ["vad", "dad", "dev"]:  # Common misrecognitions
                        name = "Daveydrz"
    ...
    if not voice_saved:
        try:
            ...
            print(f"[User] ❌ Text embedding error: {text_err}")
            # Emergency fallback
            known_users[name] = [0.0] * 384  # SentenceTransformer default size
    ...
    try:
        ...
        if name in known_users and isinstance(known_users[name], list):
            embedding_len = len(known_users[name])
            print(f"[User] Validating: {name} has {embedding_len} dimensions")
            
            # Save to file
            with open(known_users_path, "w", encoding="utf-8") as f:
                json.dump(known_users, f, indent=2, ensure_ascii=False)
            
            print(f"[User] ✅ Database saved to {known_users_path}")
            ...
            if os.path.exists(known_users_path):
                ...
                if name in verify_data:
                    print(f"[User] ✅ Verification: {name} found in saved database")
                else:
                    print(f"[User] ❌ Verification failed: {name} not in saved database")
            else:
                print(f"[User] ❌ Database file was not created!")
        else:
            print(f"[User] ❌ Invalid data for {name}: {type(known_users.get(name))}")
