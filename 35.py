...
def audio_playback_worker():
    """Update to use perfect AEC timing"""
    global current_audio_playback, vad_thread_active
    print("[Buddy][Playback Worker] Started - PERFECT AEC TIMING")
    
    audio_accumulator = []
    last_chunk_time = 0
    BUFFER_TIMEOUT = 0.06  # Optimized for AEC alignment
        ...
        try:
            try:
                ...
                if audio_accumulator:
                    _play_accumulated_audio_with_perfect_aec(audio_accumulator)  # Use new function
            ...
            if item is None:
                if audio_accumulator:
                    _play_accumulated_audio_with_perfect_aec(audio_accumulator)
            ...
            # Accumulation logic (keep existing)
            current_time = time.time()
            
            if not audio_accumulator or (current_time - last_chunk_time) < BUFFER_TIMEOUT:
                audio_accumulator.append((pcm, sr))
                last_chunk_time = current_time
            else:
                _play_accumulated_audio_with_perfect_aec(audio_accumulator)  # Use new function
                audio_accumulator = [(pcm, sr)]
                last_chunk_time = current_time
            
            playback_queue.task_done()
            
            # Flush conditions
            if playback_queue.empty() or len(audio_accumulator) >= 3:
                if audio_accumulator:
                    _play_accumulated_audio_with_perfect_aec(audio_accumulator)  # Use new function
                    audio_accumulator.clear()
...
def _play_accumulated_audio(audio_chunks):
    """Play accumulated audio chunks as one smooth stream"""
    global current_audio_playback, vad_thread_active
    ...
    try:
        # Combine all chunks into one smooth audio stream
        combined_audio = []
        target_sr = 16000
        ...
        # Create smooth continuous audio
        smooth_audio = np.concatenate(combined_audio)
        
        # Apply gentle fade in/out to prevent clicks
        fade_samples = min(160, len(smooth_audio) // 10)  # 10ms fade or 10% of audio
        if len(smooth_audio) > fade_samples * 2:
            # Fade in
            fade_in = np.linspace(0, 1, fade_samples)
            smooth_audio[:fade_samples] = (smooth_audio[:fade_samples].astype(np.float32) * fade_in).astype(np.int16)
        ...
        # Play the smooth combined audio
        with playback_lock:
            try:
                if DEBUG:
                    print(f"[Buddy][Playback] Playing SMOOTH audio: {len(smooth_audio)} samples ({len(audio_chunks)} chunks combined)")

                # Set talking flag and start VAD monitoring
                if not buddy_talking.is_set():
                    buddy_talking.set()
                    
                    # Start VAD monitoring thread only if not already active
                    if not vad_thread_active.is_set():
                        threading.Thread(target=background_vad_listener, daemon=True).start()
                        if DEBUG:
                            print("[Buddy][Playback] VAD monitoring started")

                # Update reference audio for AEC
                threading.Thread(
                    target=update_reference_audio_realtime, 
                    args=(smooth_audio, target_sr), 
                    daemon=True
                ...
                if full_duplex_interrupt_flag.is_set():
                    if DEBUG:
                        print("[Buddy][Playback] Interrupted before playing")
                    return
                ...
                while current_audio_playback.is_playing():
                    if full_duplex_interrupt_flag.is_set():
                        if DEBUG:
                            print("[Buddy][Playback] Interrupted during playback")
                        current_audio_playback.stop()
                        break
                    time.sleep(0.005)  # 5ms sleep for responsiveness
                
                current_audio_playback = None
                
                if DEBUG:
                    print("[Buddy][Playback] Finished SMOOTH playback")

            except Exception as e:
                print(f"[Buddy][Playback ERROR] Smooth playback error: {e}")
                if current_audio_playback:
                    try:
                        current_audio_playback.stop()
                    except:
                ...
                # Only clear talking flag if queue is empty
                if playback_queue.empty():
                    buddy_talking.clear()
                    if DEBUG:
                        print("[Buddy][Playback] All audio finished, ready for input")
                        
    except Exception as e:
        print(f"[Buddy][Playback ERROR] Audio combination error: {e}")
...
# ========== VAD + LISTEN ==========
def vad_and_listen():
    """Improved VAD with faster response"""
    ...
    min_speech_frames = 5  # Reduced from 10
    silence_thresh = 0.4   # Reduced from 1.0 - much faster!
    max_recording_time = 6  # Reduced from 8
    ...
    with sd.InputStream(device=MIC_DEVICE_INDEX, samplerate=MIC_SAMPLE_RATE, 
        ...
        while True:
            ...
            for i in range(0, len(mic_16k), 160):
                ...
                # Volume threshold - ignore very quiet audio
                ...
                if is_speech:
                    ...
                    # Start recording when we detect enough speech
                    if speech_detected >= min_speech_frames:
                        print("[Buddy] 🎤 Recording started...")
                        audio = frame_buffer.copy()
                        last_speech = time.time()
                        start_time = time.time()
                        frame_buffer.clear()
                        
                        # Continue recording with faster silence detection
                        while (time.time() - last_speech) < silence_thresh and (time.time() - start_time) < max_recording_time:
                            ...
                            for j in range(0, len(mic_16k), 160):
                                ...
                                if volume2 < 300:  # Lower threshold during recording
                                    continue

                                audio.append(cleaned_chunk2)

                                # Update last speech time if we detect speech
                                if vad.is_speech(cleaned_chunk2.tobytes(), 16000):
                                    last_speech = time.time()
                                    print(".", end="", flush=True)  # Visual feedback

                        print(f"\n[Buddy] 📤 Recording complete ({time.time() - start_time:.1f}s)")
...
def fast_listen_and_transcribe(history):
    wait_after_buddy_speaks(delay=0.3)  # ✅ Minimal delay to preserve full-duplex
    ...
    if not text or len(cleaned) < 2:
        ...
        return "..."  # Fallback string to avoid infinite silence loop
...
def ask_llama3_openai_streaming(messages, model="llama3", max_tokens=80, temperature=0.6, lang="en", style=None):
    """
    Optimized streaming function with immediate audio generation
    """
    global current_stream_id, spoken_chunks_cache
    
    # Increment stream ID for this request
    current_stream_id += 1
    ...
    try:
        ...
        with requests.post(url, json=payload, stream=True, timeout=30) as response:
            ...
            def should_speak_chunk(text, min_words=4):
                """Determine if chunk is ready to speak"""
                if not text or len(text.strip()) < 3:
                    return False
                ...
                # Check for sentence endings or natural breaks
                return (text.rstrip().endswith(('.', '!', '?', ',')) or 
                       len(words) >= 8 or 
                       any(word in text.lower() for word in ['and', 'but', 'so', 'because']))

            def speak_chunk_immediately(chunk_text, stream_id):
                """Generate and queue audio immediately"""
                nonlocal first_chunk_spoken
                
                if stream_id != current_stream_id:
                    if DEBUG:
                        print(f"[Stream] Skipping stale chunk from stream {stream_id}")
                    return
                ...
                if DEBUG:
                    print(f"[Stream] Speaking chunk: '{chunk_text}'")
                
                # Generate TTS immediately using direct generation
                try:
                    ...
                    if pcm is not None and sr:
                        ...
                        if DEBUG:
                            print(f"[Stream] Audio queued for: '{chunk_text[:30]}...'")
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    if delta:
                        print(delta, end="", flush=True)  # Show streaming text
                        buffer += delta
                        full_response += delta
                        
                        # Check if we should speak this chunk
                        if should_speak_chunk(buffer):
                            speak_chunk_immediately(buffer, stream_id)
                            buffer = ""
                            
                except json.JSONDecodeError as json_err:
                    if DEBUG:
                        print(f"[Stream] JSON decode error: {json_err}")
                    continue
                except Exception as line_err:
                    if DEBUG:
                        print(f"[Stream] Line processing error: {line_err}")
                    continue
            ...
            # If nothing was spoken and we have content, speak it all
            if not first_chunk_spoken and full_response.strip() and stream_id == current_stream_id:
                if DEBUG:
                    print(f"[Stream] Speaking complete response as fallback")
                speak_chunk_immediately(full_response.strip(), stream_id)

            print()  # New line after streaming text
            return full_response.strip()

    except requests.exceptions.Timeout:
        print(f"[Stream] Timeout for stream {stream_id}")
        raise
    except requests.exceptions.ConnectionError:
        print(f"[Stream] Connection error for stream {stream_id}")
        raise
    except Exception as e:
        print(f"[Stream] Unexpected error in stream {stream_id}: {e}")
        raise
...
def handle_user_interaction(speaker, history, conversation_mode="auto"):
    """
    Optimized user interaction handler for true full-duplex streaming
    """
    
    # Minimal delay to ensure clean audio state
    if buddy_talking.is_set():
        # Wait for current audio to finish, but don't block long
        timeout = time.time() + 0.5  # Max 500ms wait
        while buddy_talking.is_set() and time.time() < timeout:
            time.sleep(0.01)
    
    if DEBUG:
        print("[Buddy] Ready for input - listening...")
    ...
    # Handle interruption case first
    if full_duplex_interrupt_flag.is_set():
        if DEBUG:
            print("[Buddy] Handling interrupt - processing new input")
        full_duplex_interrupt_flag.clear()
    ...
    if is_noise_or_gibberish(question):
        if DEBUG:
            print(f"[Buddy] Filtered out gibberish: {question!r}")
        return True

    # Handle interrupt commands immediately
    INTERRUPT_PHRASES = ["stop", "buddy stop", "przerwij", "cancel", "shut up", "quiet"]
    if any(phrase in question.lower() for phrase in INTERRUPT_PHRASES):
        if DEBUG:
            print(f"[Buddy] Interrupt command received: {question}")
        stop_playback()
    ...
    try:
        if os.path.exists("temp_input.wav"):
            ...
            if new_speaker and new_speaker != speaker and confidence > 0.82:
                ...
                name = name_response.strip().title() if name_response else f"User{int(time.time())}"
                
                # Register new user
                audio_embedding = generate_embedding_from_audio(audio_np)
        ...
        if DEBUG:
            print(f"[Buddy] Speaker detection error: {e}")

    # Language detection
    try:
        # Quick heuristic for common English phrases
        common_en = ["how are you", "what is", "who are you", "tell me", "can you", "what's"]
        if any(question.lower().startswith(phrase) for phrase in common_en):
            lang = "en"
        else:
    ...
    if DEBUG:
        print(f"[Buddy] Language: {lang}")

    # Play subtle chime to acknowledge input
    try:
        play_chime()
    except Exception as e:
        if DEBUG:
    ...
    # Update memories and context BEFORE processing response
    ...
    # Main LLM processing with streaming
    if DEBUG:
    ...
    if DEBUG:
        print(f"[TIMING] LLM processing time: {time.time() - llm_start_time:.2f}s")

    # Occasional flavor responses (reduced frequency to avoid spam)
    if random.random() < 0.1:  # Reduced from 0.2 to 0.1
        flavor_line = flavor_response(ctx)
        # Small delay to avoid overlapping with main response
        threading.Timer(1.0, lambda: speak_async(flavor_line, lang)).start()
